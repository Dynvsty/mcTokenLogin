...
import xyz.villainsrule.utils.APIUtils.ProfileInfo;
    ...
    protected void init() {
        ...
        loginButton = Button.builder(Component.literal("Login"), button -> {
            ...
            if (!sessionInput.isEmpty()) {
                try {
                    ProfileInfo sessionInfo = APIUtils.getProfileInfo(sessionInput);

                    User user = SessionUtils.createSession(sessionInfo.ign(), sessionInfo.uuid(), sessionInput);
                    SessionUtils.setSession(user);

                    this.currentTitle = Component.literal("Logged in as: " + sessionInfo.ign()).withStyle(ChatFormatting.GREEN);
                    restoreButton.active = true;
                } catch (IOException | RuntimeException e) {
                    this.currentTitle = Component.literal("Invalid Session ID").withStyle(ChatFormatting.RED);
                }
            } else
                this.currentTitle = Component.literal("Session ID cannot be empty").withStyle(ChatFormatting.RED);
        ...
        if (TokenLoginMod.currentSession.equals(TokenLoginMod.restorableSession))
            restoreButton.active = false;
    }
